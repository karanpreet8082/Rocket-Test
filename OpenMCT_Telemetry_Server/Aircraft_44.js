// telemetry source object for the aircraft_42_44
...
function aircraft_42_44() {

	
	// Initialize working Parameters and Object

	// read the keys from dictionary
	let rawDict = fs.readFileSync('../openmct/example/aircraft_42_44/aircraft_42_44dictionary.json')
			...
			try{ // store in history
				...
				if(this.continousLogging){					
					...
					this.asyncStringyify(message).then(function(write) {//write is the value of the resolved promise in asyncStringify
						fs.appendFile(__dirname + '/saved_logs/aircraft_42_44_'+this.FileTimestamp+'_rawMessage.json', write, (err) => {
							if (err) {
								console.log(err);
							} 
						}) 
	...
	server.on('error', (err) => {
		console.log(`aircraft_42_44 UDP server error:\n${err.stack}`);
		try{
    ...
    console.log("aircraft_42_44 initialized!");
};


// to update every time new data comes in
aircraft_42_44.prototype.generateRealtimeTelemetry = function () {
...
aircraft_42_44.prototype.generateIntervalTelemetry = function () {
...
aircraft_42_44.prototype.notify = function (point) {
...
aircraft_42_44.prototype.listen = function (listener) {
...
aircraft_42_44.prototype.SetFileTimestamp = function () {
...
aircraft_42_44.prototype.asyncStringyify = function (obj) {
...
aircraft_42_44.prototype.command = function (command) {
	...
	if(command === ':saveHistory'){
		...
		this.asyncStringyify(this.history).then(function(write) {//write is the value of the resolved promise in asyncStringify
			fs.writeFile(__dirname + '/saved_logs/aircraft_42_44_'+this.FileTimestamp+'_History.json', write, (err) => {
				if (err) {
					console.log(err);
				} else {
	...
	// 			fs.writeFile(__dirname + '/saved_logs/aircraft_42_44_'+this.FileTimestam+'.json', write, (err) => {
	// 				if (err) {
	// 					throw err;
	// 				}
